...
async def an(e):
    ...
    saka_bot.add_handler(notes, events.NewMessage())


@saka_cmd(pattern="[Rr][m]( (.*)|$)")
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    user = e.sender_id
    if not wrd:
