...
from Saka.dB import DEVS
from Saka.dB.logusers_db import *
from Saka.dB.pmpermit_db import *
...
if udB.get_key("PMSETTING"):
    if udB.get_key("AUTOAPPROVE"):
        ...
        async def autoappr(e):
            ...
            try:
                await saka_bot.edit_folder(miss.id, folder=0)
    ...
    async def permitpm(event):
        ...
        if not is_approved(user.id) and event.text != UND:
            if Redis("MOVE_ARCHIVE"):
                try:
                    await saka_bot.edit_folder(user.id, folder=1)
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm:
                        results = await saka_bot.inline_query(my_bot, f"ip_{user.id}")
                        try:
                        ...
                        _to_delete[user.id] = await saka_bot.send_file(
                            user.id,
                            PMPIC,
                            caption=message_,
                        )
                    else:
                        _to_delete[user.id] = await saka_bot.send_message(
                            user.id, message_
                        )
                    ...
                    if inline_pm:
                        try:
                            results = await saka_bot.inline_query(
                ...
                if inline_pm:
                    ...
                    _to_delete[user.id] = await saka_bot.send_message(user.id, message_)
            LASTMSG.update({user.id: event.text})
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await saka_bot(BlockRequest(user.id))
                await saka_bot(ReportSpamRequest(peer=user.id))
                await asst.edit_message(
                    int(udB.get_key("LOG_CHANNEL")),
                    _not_approved[user.id],
    ...
    @saka_cmd(pattern="(start|stop|clear)archive$", fullsudo=False)
    async def _(e):
    ...
    @saka_cmd(pattern="[oO][kK](?: |$)", fullsudo=False)
    ...
    @saka_cmd(pattern="[nN][oO](?: |$)", fullsudo=False)
...
@saka_cmd(pattern="[Bb]lock( (.*)|$)", fullsudo=False)
...
@saka_cmd(pattern="[Uu]nblock( (.*)|$)", fullsudo=False)
...
@saka_cmd(pattern="[Ll]istok", owner=True)
async def list_approved(event):
    ...
    for i in all:
        try:
            name = get_display_name(await saka_bot.get_entity(i))
        except BaseException:
            name = ""
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await saka_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user = await saka_bot.get_entity(uid)
        except BaseException:
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await saka_bot(BlockRequest(uid))
    except BaseException:
        pass
    try:
        user = await saka_bot.get_entity(uid)
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    try:
        await saka_bot(UnblockRequest(uid))
    except BaseException:
        pass
    try:
        user = await saka_bot.get_entity(uid)
...
async def ytfuxist(e):
    try:
        ...
        await saka_bot.delete_messages(e.chat_id, e.id)
...
async def in_pm_ans(event):
    ...
    await event.answer(res, switch_pm="• Saka •", switch_pm_param="start")


@callback(re.compile("admin_only(.*)"), from_users=[saka_bot.uid])
