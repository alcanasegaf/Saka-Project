...
@saka_cmd(pattern=r"^[Tt][e][r][k][a][d][a][n][g]$")
async def _(event):
...
@saka_cmd(pattern=r"^[Mm][f]$")
async def _(event):
    await eor(event, "`mf g dl` **ミ(ノ;_ _)ノ=3** ")


@saka_cmd(pattern=r"^[Cc][i][n][t][a]$")
async def _(event):
    animation_chars = [
...
@saka_cmd(pattern=r"^[Gg][o][m][b][a][l]$")
async def _(event):
...
@saka_cmd(pattern="^[Hh][e][l][i][k][o][p][t][e][r]$")
async def _(event):
    await eor(
        event,
        "▬▬▬.◙.▬▬▬ \n"
...
@saka_cmd(pattern=r"^[Bb][e][r][n][y][a][n][y][i]$")
async def _(event):
