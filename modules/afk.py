...
from Saka.dB.afk_db import add_afk, del_afk, is_afk
from Saka.dB.pmpermit_db import is_approved
from telegraph import upload_file as uf
from telethon import events

from . import (LOG_CHANNEL, NOSPAM_CHAT, Redis, asst, saka_bot, saka_cmd,
               get_string, mediainfo, udB)

old_afk_msg = []


@saka_cmd(pattern="^[Aa][Ff][Kk]( (.*)|$)", owner_only=True)
async def set_afk(event):
    ...
    saka_bot.add_handler(remove_afk, events.NewMessage(outgoing=True))
    saka_bot.add_handler(
        on_afk,
        events.NewMessage(
    ...
    if text and media:
        if "sticker" in media_type:
            msg1 = await saka_bot.send_file(event.chat_id, file=media)
            msg2 = await saka_bot.send_message(
                event.chat_id, get_string("afk_5").format(text)
            )
        else:
            msg1 = await saka_bot.send_message(
                event.chat_id, get_string("afk_5").format(text), file=media
            )
    elif media:
        if "sticker" in media_type:
            msg1 = await saka_bot.send_file(event.chat_id, file=media)
            msg2 = await saka_bot.send_message(event.chat_id, get_string("afk_6"))
        else:
            msg1 = await saka_bot.send_message(
