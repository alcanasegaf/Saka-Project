...
async def mentionall(event):
    ...
    is_admin = False
    try:
        partici_ = await saka_bot(GetParticipantRequest(event.chat_id, event.sender_id))
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in saka_bot.iter_participants(chat_id):
        ...
        if usrnum == 5:
            if mode == "teks":
                txt = f"{usrtxt}\n\n{msg}"
                await saka_bot.send_message(chat_id, txt)
            elif mode == "balas":
...
@saka_cmd(pattern="[Ee][m][o][j][i][t][a][g](?: |$)(.*)")
