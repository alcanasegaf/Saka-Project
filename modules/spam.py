...
@saka_cmd(pattern="[tT]spam", fullsudo=False)
async def tmeme(e):
    tspam = str(e.text[7:])
...
@saka_cmd(pattern="[sS]pam", fullsudo=False)
async def spammer(e):
    message = e.text
    if e.reply_to:
...
@saka_cmd(pattern="[bB]spam", fullsudo=True)
async def bigspam(e):
    message = e.text
    if e.reply_to:
        if len(message.split()) < 2:
...
@saka_cmd(pattern="[dD]spam?(.*)", fullsudo=False)
