...
async def pack_kangish(_):
    ...
    try:
        short_name = "saka_" + _packname.replace(" ", "_") + str(_.id)
        _r_e_s = await asst(
...
async def hehe(args):
    ...
    if message.photo:
        photo = io.BytesIO()
        photo = await saka_bot.download_media(message.photo, photo)
    elif message.file and "image" in message.file.mime_type.split("/"):
        photo = io.BytesIO()
        await saka_bot.download_file(message.media.document, photo)
        if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🗿"
        if len(splat) == 3:
            pack = splat[2]  # User sent saka_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with saka_bot.conversation("@Stickers") as conv:
                try:
                    await conv.send_message("/addsticker")
                except YouBlockedUserError:
                    LOGS.info("Unblocking @Stickers for kang...")
                    await saka_bot(functions.contacts.UnblockRequest("stickers"))
                    await conv.send_message("/addsticker")
                ...
                await conv.get_response()
                await saka_bot.send_read_acknowledge(conv.chat_id)
        else:
