...
try:
    ...
    from Saka.fns.tools import safe_load
try:
...
@saka_cmd(pattern="^[Bb][Aa][sS][hH]", fullsudo=False, only_devs=False)
async def _(event):
...
@saka_cmd(pattern="^[Ee][vV][Aa][lL]", fullsudo=False, only_devs=False)
...
@saka_cmd(pattern="cpp", only_devs=False)
async def doie(e):
    ...
    m = await bash("g++ -o CppSaka cpp-ayra.cpp")
    o_cpp = f"• **Eval-Cpp**\n`{match}`"
    if m[1]:
        o_cpp += f"\n\n**• Error :**\n`{m[1]}`"
        if len(o_cpp) > 3000:
            os.remove("cpp-ayra.cpp")
            if os.path.exists("CppSaka"):
                os.remove("CppSaka")
