...
from . import saka_cmd, eod, eor, get_string, mediainfo
...
@saka_cmd(pattern="[Ss]etbio( (.*)|$)", fullsudo=False)
async def _(ayra):
    ok = await ayra.eor("...")
...
@saka_cmd(pattern="[Ss]etname ?((.|//)*)", fullsudo=False)
async def _(ayra):
    ok = await ayra.eor("...")
...
@saka_cmd(pattern="[sS]etfp$", fullsudo=False)
async def _(ayra):
...
@saka_cmd(pattern="[dD]elfp( (.*)|$)", fullsudo=False)
