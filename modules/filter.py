...
from Saka.dB.filter_db import add_filter, get_filter, list_filter, rem_filter
from Saka.fns.tools import create_tl_btn, format_btn, get_msg_button
from telegraph import upload_file as uf
from telethon.tl.types import User
from telethon.utils import pack_bot_file_id

from . import saka_bot, saka_cmd, events, get_string, mediainfo, udB
from ._inline import something


@saka_cmd(pattern="[Aa]ddfil( (.*)|$)")
async def af(e):
    ...
    saka_bot.add_handler(filter_func, events.NewMessage())


@saka_cmd(pattern="[Dd]elfil( (.*)|$)")
async def rf(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
