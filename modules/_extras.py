...
from Saka.dB import stickers
from Saka.dB.forcesub_db import get_forcesetting
from Saka.dB.gban_mute_db import is_gbanned
from Saka.dB.greetings_db import get_goodbye, get_welcome, must_thank
from Saka.fns.helper import inline_mention
from Saka.fns.tools import create_tl_btn
...
try:
    ...
    detector = None
from . import LOG_CHANNEL, LOGS, asst, saka_bot, get_string, types, udB
from ._inline import something


@saka_bot.on(events.ChatAction())
...
async def DummyHandler(bjir):
    ...
    if (
        ...
        if not user.bot:
            ...
            try:
                await saka_bot(GetParticipantRequest(int(joinchat), user.id))
            except UserNotParticipantError:
                await saka_bot.edit_permissions(
                    bjir.chat_id, user.id, send_messages=False
                )
                res = await saka_bot.inline_query(
...
"""
@saka_bot.on(events.NewMessage(incoming=True))
...
@saka_bot.on(events.Raw(types.UpdateUserName))
