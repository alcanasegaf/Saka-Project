...
from Saka.dB.blacklist_db import (add_blacklist, get_blacklist, list_blacklist,
                                  rem_blacklist)

from . import saka_bot, saka_cmd, events, get_string, udB


@saka_cmd(pattern="^[Bb][l]( (.*)|$)")
async def af(e):
    ...
    saka_bot.add_handler(blacklist, events.NewMessage(incoming=True))
    await e.eor(get_string("blk_2").format(x))


@saka_cmd(pattern="^[Ww][l]( (.*)|$)")
async def rf(e):
