...
@saka_cmd(pattern=r"^[aA][lL][iI][vV][eE](?: |$)(.*)")
async def lol(
...
@saka_cmd(pattern="^[Pp][Ii][Nn][Gg]$", chats=[], type=["official", "assistant"])
...
@saka_cmd(pattern=r"^[uU][pP][dD][aA][tT][eE](?: |$)(.*)")
async def _(e):
    ...
    if m:
        ...
        await xx.edit(
            f'<code>Your BOT is </code><strong>up-to-date</strong><code> with </code><strong><a href="https://github.com/alcanasegaf/Saka-Userbot/tree/{branch}">[{branch}]</a></strong>',
            parse_mode="html",
            link_preview=False,
        )
