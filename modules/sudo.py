...
from Saka._misc import sudoers
from telethon.tl.types import User

from . import saka_bot, saka_cmd, get_string, inline_mention, udB


@saka_cmd(pattern="[Aa][d][d][s][u][d][o]( (.*)|$)", fullsudo=False)
async def _(ayra):
...
@saka_cmd(pattern="[Dd][e][l][s][u][d][o]( (.*)|$)", fullsudo=False)
async def _(ayra):
    inputs = ayra.pattern_match.group(1).strip()
    if ayra.reply_to_msg_id:
        replied_to = await ayra.get_reply_message()
...
async def _(ayra):
    ...
    if not m:
        m = "[False](https://graph.org/Saka-11-29)"
