...
from Saka.dB.gban_mute_db import (gban, gmute, is_gbanned, is_gmuted,
                                  list_gbanned, ungban, ungmute)
from Saka.kynan import register
from telethon.errors.rpcerrorlist import ChatAdminRequiredError, FloodWaitError
from telethon.tl.functions.contacts import BlockRequest, UnblockRequest
from telethon.tl.types import User

from . import (LOGS, OWNER_NAME, saka_bot, saka_cmd, eod, get_string,
               inline_mention)


@saka_cmd(pattern="[uU][n][g][b][a][n]( (.*)|$)", fullsudo=False)
...
@saka_cmd(pattern="[gG][b][a][n]( (.*)|$)", fullsudo=False)
...
@saka_cmd(pattern="[Gg]mute( (.*)|$)", fullsudo=False)
async def _(e):
...
@saka_cmd(pattern="[uU]ngmute( (.*)|$)", fullsudo=False)
async def _(e):
...
async def list_gengbanned(event):
    ...
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"List of users GBanned by {inline_mention(saka_bot.me)}",
        )
