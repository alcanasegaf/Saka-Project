...
from Saka._misc._assistant import callback, in_pattern
from Saka.dB._core import HELP, LIST
from Saka.fns.helper import gen_chlog, time_formatter, updater
from Saka.fns.misc import split_list
...
async def _(event):
    ...
    if len(changelog_str) > 1024:
        ...
        with open("saka_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await event.edit(
            get_string("upd_5"),
            file="saka_updates.txt",
            buttons=[
                [Button.inline("Update Sekarang", data="updatenow")],
                [Button.inline("❮", data="ownr")],
            ],
        )
        remove("saka_updates.txt")
    else:
        await event.edit(
            changelog_str,
            buttons=[
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Saka Op", text=txt, link_preview=False, buttons=btn)
    ]
