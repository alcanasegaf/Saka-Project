...
async def autoupdate_local_database():
    from .. import Var, asst, saka_bot, udB

    global db_url
    db_url = udB.get_key("TGDB_URL") or Var.TGDB_URL or saka_bot._cache.get("TGDB_URL")
    if db_url:
        _split = db_url.split("/")
        _channel = _split[-2]
        _id = _split[-1]
...
async def autobot():
    from .. import saka_bot, udB

    if udB.get_key("BOT_TOKEN"):
        return
    await saka_bot.start()
    ...
    if who.username:
        ...
        username = "saka_" + (str(who.id))[5:] + "_bot"
    bf = "@BotFather"
    await saka_bot(UnblockRequest(bf))
    await saka_bot.send_message(bf, "/cancel")
    await asyncio.sleep(1)
    await saka_bot.send_message(bf, "/newbot")
    await asyncio.sleep(1)
    isdone = (await saka_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("That I cannot do.") or "20 bots" in isdone:
        ...
        sys.exit(1)
    await saka_bot.send_message(bf, name)
    await asyncio.sleep(1)
    isdone = (await saka_bot.get_messages(bf, limit=1))[0].text
    if not isdone.startswith("Good."):
        await saka_bot.send_message(bf, "My Assistant Bot")
        await asyncio.sleep(1)
        isdone = (await saka_bot.get_messages(bf, limit=1))[0].text
        if not isdone.startswith("Good."):
            LOGS.critical(
            ...
            sys.exit(1)
    await saka_bot.send_message(bf, username)
    await asyncio.sleep(1)
    isdone = (await saka_bot.get_messages(bf, limit=1))[0].text
    await saka_bot.send_read_acknowledge("botfather")
    if isdone.startswith("Sorry,"):
        ran = randint(1, 100)
        username = "saka_" + (str(who.id))[6:] + str(ran) + "_bot"
        await saka_bot.send_message(bf, username)
        await asyncio.sleep(1)
        isdone = (await saka_bot.get_messages(bf, limit=1))[0].text
    if isdone.startswith("Done!"):
        ...
        await enable_inline(saka_bot, username)
        LOGS.info(
...
async def autopilot():
    from .. import asst, saka_bot, udB

    channel = udB.get_key("LOG_CHANNEL")
    new_channel = None
    if channel:
        try:
            chat = await saka_bot.get_entity(channel)
    ...
    if not channel:
        ...
        async def _save(exc):
            udB._cache["LOG_CHANNEL"] = saka_bot.me.id
            await asst.send_message(
                saka_bot.me.id, f"Gagal Membuat Saluran Log karena {exc}.."
            )
        ...
        try:
            r = await saka_bot(
                CreateChannelRequest(
                    title="Saka-Userbot Logs",
                    about="Ini adalah grup logs dari Saka-Userbot\nJangan keluar dari grup logs ini\nPowered By @GokilTeam",
                    megagroup=True,
                ),
            )
    ...
    assistant = True
    try:
        await saka_bot.get_permissions(int(channel), asst.me.username)
    except UserNotParticipantError:
        try:
            await saka_bot(InviteToChannelRequest(int(channel), [asst.me.username]))
        except BaseException as er:
    ...
    if isinstance(chat.photo, ChatPhotoEmpty):
        photo = await download_file(
            "https://telegra.ph//file/710f077579414ee279894.jpg", "logo.jpg"
        )
        ll = await saka_bot.upload_file(photo)
        try:
            await saka_bot(EditPhotoRequest(int(channel), InputChatUploadedPhoto(ll)))
        except BaseException as er:
            LOGS.exception(er)
        os.remove(photo)
...
async def customize():
    ...
    try:
        ...
        if not saka_bot.me.username:
            sir = saka_bot.me.first_name
        else:
            sir = f"@{saka_bot.me.username}"
        file = random.choice(
            [
                "https://telegra.ph//file/710f077579414ee279894.jpg",
        ...
        await asyncio.sleep(1)
        await saka_bot.send_message("botfather", "/cancel")
        await asyncio.sleep(1)
        await saka_bot.send_message("botfather", "/setuserpic")
        await asyncio.sleep(1)
        isdone = (await saka_bot.get_messages("botfather", limit=1))[0].text
        if isdone.startswith("Invalid bot"):
            LOGS.info("Error while trying to customise assistant, skipping...")
            return
        await saka_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await saka_bot.send_file("botfather", file)
        await asyncio.sleep(2)
        await saka_bot.send_message("botfather", "/setabouttext")
        await asyncio.sleep(1)
        await saka_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await saka_bot.send_message(
            "botfather", f"✨ Hello ✨!! I'm Assistant Bot of {sir}"
        )
        await asyncio.sleep(2)
        await saka_bot.send_message("botfather", "/setdescription")
        await asyncio.sleep(1)
        await saka_bot.send_message("botfather", UL)
        await asyncio.sleep(1)
        await saka_bot.send_message(
            "botfather",
            f"✨ Powerful Saka-Userbot Assistant  ✨\n✨ Master ~ {sir} ✨\n\n✨ Powered By ~ @GokilTeam ✨",
        )
        await asyncio.sleep(2)
...
async def plug(plugin_channels):
    ...
    for chat in plugin_channels:
        LOGS.info(f"{'•'*4} {chat}")
        try:
            async for x in saka_bot.iter_messages(
                ...
                try:
                    ...
                    LOGS.info(f"Saka-Userbot - PLUGIN_CHANNEL - ERROR - {plugin}")
                    LOGS.exception(e)
                    os.remove(plugin)
        except Exception as er:
            LOGS.exception(er)
...
async def ready():
    ...
    if not udB.get_key("INIT_DEPLOY"):  # Detailed Message at Initial Deploy
        MSG = """ **Thanks for Deploying Saka-Userbot!**
• Here, are the Some Basic stuff from, where you can Know, about its Usage."""
        PHOTO = "https://telegra.ph//file/710f077579414ee279894.jpg"
        ...
        if prev_spam:
            try:
                await saka_bot.delete_messages(chat_id, int(prev_spam))
            except Exception as E:
                LOGS.info(
    ...
    try:
        ...
        try:
            await (await saka_bot.send_message(chat_id, str(e))).delete()
            spam_sent = await asst.send_message(chat_id, MSG, file=PHOTO, buttons=BTTS)
        ...
        LOGS.info(el)
        try:
            spam_sent = await saka_bot.send_message(chat_id, MSG)
...
async def WasItRestart(udb):
    ...
    if not key:
        return
    from .. import asst, saka_bot

    try:
        data = key.split("_")
        who = asst if data[0] == "bot" else saka_bot
...
async def enable_inline(saka_bot, username):
    bf = "BotFather"
    await saka_bot.send_message(bf, "/setinline")
    await asyncio.sleep(1)
    await saka_bot.send_message(bf, f"@{username}")
    await asyncio.sleep(1)
    await saka_bot.send_message(bf, "Search")
    await saka_bot.send_read_acknowledge(bf)
