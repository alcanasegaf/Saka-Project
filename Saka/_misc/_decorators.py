...
from ..version import saka_version as saka_ver
...
def saka_cmd(pattern=None, manager=False, saka_bot=saka_bot, asst=asst, **kwargs):
    ...
    def decor(dec):
        async def wrapp(ay):
            ...
            try:
                ...
                await saka_bot.disconnect()
                await asyncio.sleep(fwerr.seconds + 10)
                await saka_bot.connect()
                await asst.send_message(
                    udB.get_key("LOG_CHANNEL"),
                ...
                ftext = "**Saka Client Error:** `Forward this to` @kynansupport\n\n"
                ftext += "**Saka Version:** `" + str(pyver)
                ftext += "`\n**Userbot Version:** `" + str(saka_ver)
                ...
                if len(ftext) > 4096:
                    with BytesIO(ftext.encode()) as file:
                        ...
                        error_log = await asst.send_file(
                            udB.get_key("LOG_CHANNEL"),
                            file,
                            caption="**Saka Client Error:** `Forward this to` @kynansupport\n\n",
                        )
                else:
