...
from Saka import run_as_module
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "© Kynan"
            await eod(ok, f"✓ `Saka-Userbot - Installed`: `{plug}` ✓\n\n{output}")
    ...
    async def heroku_logs(event):
        ...
        await event.client.send_file(
            ...
            caption="**Saka-Userbot Heroku Logs.**",
        )
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Saka-Userbot {saka_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Saka-Userbot {saka_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
...
async def restart(ay=None):
    if Var.HEROKU_APP_NAME and Var.HEROKU_API:
        ...
        if len(sys.argv) == 1:
            os.execl(sys.executable, sys.executable, "-m", "Saka")
        else:
            os.execl(
                sys.executable,
                sys.executable,
