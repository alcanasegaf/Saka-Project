...
from Saka._misc import SUDO_M, owner_and_sudos
from Saka.dB.asst_fns import *
...
async def own(event):
    msg = Owner_info_msg.format(
        mention=event.sender.mention, me=inline_mention(saka_bot.me)
    )
    if custom_info:
...
async def ayra(event):
    ...
    if event.sender_id not in SUDO_M.fullsudos:
        ...
        if not udB.get_key("STARTMSG"):
            ...
            await event.reply(
                f"Hey {mention}, Aku Adalah Saka Asissten {me}!\n\n{ok}",
...
async def botstat(event):
    ...
    msg = """Saka-Userbot Assistant - Stats
Total Users - {}""".format(
        ok,
    )
